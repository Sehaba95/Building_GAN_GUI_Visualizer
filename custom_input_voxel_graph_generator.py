...
print("FILENAME: {}".format(input_values["filename"]))

def add_neighor(i, j, k, x, y, z):
    if  i >= 0 and j >= 0 and k >= 0 and i < x and j < y and k < z:
        return True

    return False

def get_neighbors(i, j, k, x, y, z):
    # Get as arguments : the current voxel id [k, j, i] and the maximum values of all the three axis 
    neighbors = []

    if add_neighor(i-1, j, k, x, y, z):
        neighbors.append([k, j, i-1])

    if add_neighor(i+1, j, k, x, y, z):
        neighbors.append([k, j, i+1])

    if add_neighor(i, j-1, k, x, y, z):
        neighbors.append([k, j-1, i])

    if add_neighor(i, j+1, k, x, y, z):
        neighbors.append([k, j+1, i])

    if add_neighor(i, j, k-1, x, y, z):
        neighbors.append([k-1, j, i])
    ...
    for floor in range(number_of_floors):
        ...
        for dim_y in input_values["y_intervals"]:
            ...
            for dim_x in input_values["x_intervals"]:
                ...
                # Give as arguments : the current voxel id [z, y, x] and the maximum values of all the three axis 
                neighbors = get_neighbors(x, y, floor, len(input_values["x_intervals"]), len(input_values["y_intervals"]), number_of_floors)
