...
#print(args.input_object)
#print(args.variation_num)

inference_id = "output" + args.input_object.split("/")[-1].split(".")[0].replace("data","")
...
trained_file = 'runs/iccv2021/checkpoints/70.ckpt'

# outputs/, 'output_2023_09_07_16_57_09_323568/'
viz_dir = os.path.join(args.output_dir, inference_id + "/")
...
#variation_test_data1 = torch.load(os.path.join(args.train_data_dir, data_fname_list[0]))
...
# Load the processed input (global graph, local graph, and voxel graph)
input_object = torch.load(args.input_object)
variation_test_batch1 = Batch.from_data_list([input_object for _ in range(args.variation_num)], follow_batch)
...
program_input_dim = input_object.program_class_feature.size(-1) + 1  # data_list[0].story_level_feature.size(-1)
voxel_input_dim = input_object.voxel_feature.size(-1)
voxel_label_dim = input_object.voxel_label.size(-1)
...
generate_multiple_outputs_from_batch(variation_test_batch1, args.variation_num, generator, args.raw_dir, viz_dir, follow_batch, device_ids, trunc=trunc_num)
